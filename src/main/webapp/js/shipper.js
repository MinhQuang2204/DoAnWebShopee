		...
		success: function(result) {
			if (result.length == 0) {
				...
				for (var i = 0; i < result.length; i++) {
					...
					if(s == 7){
						html += `<td><button id="orderStatus" onclick="editStatus(this, ${result[i].order.orderId})" value="${result[i].id}">Nhận đơn</button></td>`
					}
					if (s == 2) {
						html += `<td><button id="orderStatus" onclick="editStatus(this, ${result[i].order.orderId})" value="${result[i].id}">Xác nhận lấy hàng</button></td>`
...
function editStatus(btn, param) {
	let id = btn.value;
	let statusName = btn.textContent;
	//alert(param);
		$.ajax({
		url: "/Shopee/statusshipperorder", //send to Controller
